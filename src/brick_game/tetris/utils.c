...
void checkFullLines() {
  ...
  for (int i = TETRIS_FIELD_H - 1; i > 0; i--) {
    int count_blocks = 0;
    for (int j = 0; j < TETRIS_FIELD_W; j++) {
      count_blocks += state->field[i][j];
    }

    if (count_blocks == TETRIS_FIELD_W) {
      for (int z = i; z > 0; z--) {
        for (int j = 0; j < TETRIS_FIELD_W; j++) {
...
void updateLevelAndSpeed() {
  ...
  int new_level = state->score / TETRIS_MAX_SCORE_IN_LEVEL + 1;
  if (new_level > MAX_LEVEL) new_level = MAX_LEVEL;

  state->level = new_level;
  state->speed = TETRIS_INIT_SPEED - (state->level - 1) * TETRIS_SPEED_STEP;
}
...
void freeMemory() {
  ...
  freeMatrix(state->field, TETRIS_FIELD_H);
