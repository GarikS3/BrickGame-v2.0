...
void spawnFigure() {
  ...
  state->fig_x = TETRIS_FIELD_W / 2 - FIGURE_SIZE / 2;
  state->fig_y = -1;
...
void attachFigureOnField() {
  ...
  for (int i = 0; i < FIGURE_SIZE; i++) {
    for (int j = 0; j < FIGURE_SIZE; j++) {
      ...
      if ((new_x >= 0) && (new_x < TETRIS_FIELD_W) && (new_y >= 0) &&
          (new_y < TETRIS_FIELD_H) && (state->figure[i][j] == 1)) {
...
int **generateNewFigure() {
  ...
  Figure_t figure_index = (Figure_t)(rand() % NUM_FIGURES);
